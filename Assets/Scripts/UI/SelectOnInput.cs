...
public class SelectOnInput : MonoBehaviour {
    ...
    private bool directionCheck()
    {
        if (horizontalControlled && verticalControlled)
            return Input.GetAxisRaw("DPadHorizontal") != 0 || Input.GetAxisRaw("DPadVertical") != 0;
        else if (horizontalControlled)
            return Input.GetAxisRaw("DPadHorizontal") != 0;
        else if (verticalControlled)
            return Input.GetAxisRaw("DPadVertical") != 0;
